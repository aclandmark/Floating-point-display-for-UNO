...
unsigned char check_for_OVF(float Fnum){     													//Subroutine crashes if result exceeeds FP bounds                
  ...
  eeprom_write_byte(OVF_cntl_reg,
 eeprom_read_byte(OVF_cntl_reg) & (~(1 << OVF_test)));				//FE		//Clear bit 0: Remains zero if the FPN exceeds its bounds
  
  if (Fnum < 0){sign = '-'; Fnum *= (-1);}                  							//Convert negative numbers to positive ones and set the sign character
  wdt_enable(WDTO_30MS);
  
  if (*long_ptr == 0x7F800000) while(1);
  if (*long_ptr == 0x8F800000) while(1);
  if (*long_ptr == 0X0) {return 1;}
  if (*long_ptr == 0X80000000) {return -1;}
  ...
  //if((eeprom_read_byte(OVF_cntl_reg))&((1 << OVF_active)|(1 << Restore_OVF))){								//Bit 1 is set: WDT has reset UNO   (~(1 << Restore_OVF)))
  if((eeprom_read_byte(OVF_cntl_reg))&(1 << OVF_active)){
  if((eeprom_read_byte(OVF_cntl_reg))&(1 << Restore_OVF)){
  
  //if((eeprom_read_byte(OVF_cntl_reg)) == 0xFF){
