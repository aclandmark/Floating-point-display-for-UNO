...
signed char Format_for_Display(char* array, char sign, signed char expt){		//Receives FP_strings with MSB or sign in location 0 (RH end of display)
	...
	int array_ptr, LSB_ptr,dp_ptr, num_dps;
...
*/
	...
	{num_dps = LSB_ptr - dp_ptr;  Serial.write('\t');Serial.write(dp_ptr+'0');Serial.write(LSB_ptr+'0');
	...
	{array[LSB_ptr] = 0;array[LSB_ptr-1] += 1;}
	LSB_ptr -= 1;Serial.write('a');}
	
	if (array[LSB_ptr] == ('.' + 1))											//if dp is incremented restore it
	{Serial.write('c');								//and increment first digit on LHS of dp
	if (array[0]=='0')expt -= 1;
	array[LSB_ptr--] -= 1;
	array[LSB_ptr] += 1;
	}
	...
	{if (array[LSB_ptr] == ':')
	{array[LSB_ptr] = '0';array[LSB_ptr-1] += 1;}
	LSB_ptr -= 1;Serial.write('b');}
	
	
	}}
	
	Serial.write("\r\n");Serial.write('C');pcb_display_to_PC(display_array);Serial.write('E');Serial.write(expt + '0');/////////////////////////
	/***********************************************************************/	
...
void ftoaL(float Fnum, char FP_string[]){
	...
	Serial.write("\r\n");Serial.write('A');
	Serial.write(FP_string);//OK to here
